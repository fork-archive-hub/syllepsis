...
class ImageMask extends React.Component<IViewMapProps<ImageAttrs>, any> {
  ...
  public renderImage = () => {
    ...
    const { src, alt, width: attrWidth } = attrs;
    let height = attrs.height;

    const config = editor.command.image!.getConfiguration();
    const width = attrWidth || Math.min(DEFAULT_IMG_SIZE.width, config.uploadMaxWidth!);
    if (isUploading && !attrWidth) height = (width * 9) / 16;
